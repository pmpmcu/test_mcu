...
{ 
	...
	CAN_FilterInitStructure.CAN_FilterNumber=13;	//过滤器0--13
	CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdList; 	//	list  模式
	CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_16bit; 	//16位宽 
	CAN_FilterInitStructure.CAN_FilterIdHigh=(can_CAN_Filter_id_1|CAN_ID_STD)<<5;	//32位ID  标准贞 一共 27位，左移5位，与高位对齐。扩展45位 左移 3位。
	CAN_FilterInitStructure.CAN_FilterIdLow=(Can_iso15765_Receive_id|CAN_ID_STD)<<5;//32位ID  标准贞 一共 27位，左移5位，与高位对齐。扩展45位 左移 3位。
	...
	CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdList; 	//list 模式
	CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_16bit; 	//16 位宽 
	CAN_FilterInitStructure.CAN_FilterIdHigh=(can_CAN_Filter_id_6|CAN_ID_STD)<<5;	//16位ID
	CAN_FilterInitStructure.CAN_FilterIdLow=(can_CAN_Filter_id_7|CAN_ID_STD)<<5; //16位ID
	...
	CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdList; 	//list 模式
	CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_16bit; 	//16 位宽 
	CAN_FilterInitStructure.CAN_FilterIdHigh=(can_CAN_Filter_id_8|CAN_ID_STD)<<5;	//16位ID
	CAN_FilterInitStructure.CAN_FilterIdLow=(can_CAN_Filter_id_9|CAN_ID_STD)<<5; //16位ID
	CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0x0000;//MASK
	CAN_FilterInitStructure.CAN_FilterMaskIdLow=0x0000;//MASK
	CAN_FilterInitStructure.CAN_FilterFIFOAssignment=CAN_Filter_FIFO0;//过滤器0关联到FIFO0
	CAN_FilterInitStructure.CAN_FilterActivation=ENABLE;//激活过滤器0
	...
	CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdList; 	//list 模式
	CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_16bit; 	//16 位宽 
	CAN_FilterInitStructure.CAN_FilterIdHigh=(can_CAN_Filter_id_10|CAN_ID_STD)<<5;	//16位ID
	CAN_FilterInitStructure.CAN_FilterIdLow=(can_CAN_Filter_id_11|CAN_ID_STD)<<5; //16位ID
	CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0x0000;//MASK
	CAN_FilterInitStructure.CAN_FilterMaskIdLow=0x0000;//MASK
	CAN_FilterInitStructure.CAN_FilterFIFOAssignment=CAN_Filter_FIFO0;//过滤器0关联到FIFO0
	CAN_FilterInitStructure.CAN_FilterActivation=ENABLE;//激活过滤器0
...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
	...
	#endif
	window_close_decode(CAN_ID_VALUE,RxMessage.Data); //关门
...
void window_close_decode(uint32_t can_id,u8 *buf)
{
	if(can_id==can_CAN_Filter_id_6) // 0x391
		{
		//if(key_lock==1) //如果已经息火
			{
			if((buf[0]==0x00)&(buf[1]==0x00)&(buf[2]==0x00)&(buf[3]==0x00)
				&(buf[4]==0x00)&(buf[5]==0x00)&(buf[6]==0x00)&(buf[7]==0x22))  // 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x01  锁车
																													// 0x22
				{
				door_close_test(); //关门
				acc_on_flag=0;
			...
			if((buf[0]==0x00)&(buf[1]==0x00)&(buf[2]==0x80)&(buf[3]==0x00)
				&(buf[4]==0x00)&(buf[5]==0x00)&(buf[6]==0x80)&(buf[7]==0xA2))//391    00 00 80 00 00 00 80 81 开锁//
																									//  80 A2
				{
				door_open_test();
				//door_close_test(); //关门
	...
	if(can_id==can_CAN_Filter_id_9) // 0x0fa  没有接收此ID  说明已经关火。
		{
		acc_off_flag=0;
		}
...
void Can_ISO15765_send_data_app(uint32_t can_id,u8 *buf)
{
	...
	if(can_id==Can_iso15765_Receive_id)
		...
		for(i=0;i<8;i++)
			{
			app_data_buf[2+i]=buf[i]; //数据复制到app_data_buf
			}
		app_data_buf[15]=data_generate_sum(app_data_buf);//最后一位，校准合。
		for(t=0;t<16;t++)
			{
				while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET);//等待发送结束
				USART_SendData(USART1, app_data_buf[t]);//向串口1发送数据
				//while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET);//等待发送结束
			}
		}
