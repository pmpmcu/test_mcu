...
#define timer_out_cnt 2000 //2000*100ms=200s
	...
	u8 canbuf_rx[8];
 	u8 can_pid_table[5]={Engine_load,Engine_RPM,Velicele_speed,Throttle_position,Battery_Voltage};
	 u8 cnt=0,res=0;
 void can_tx(void)
 {
	 u8 i=0;
...
//	u16 timer_count=timer_out_cnt;
	...
	CAN_Mode_Init(CAN_SJW_1tq,CAN_BS2_8tq,CAN_BS1_9tq,8,CAN_Mode_Normal);//所以这里CANbps=APB1总线频率36000000/4/(1+8+9))=500k bps   
	//  4  500K    8 250K
 	...
 	while(1)
	{
		...
		if(TIM3_IRQHandler_count==0)
			{
			TIM3_IRQHandler_count=1000;
			
			LED0=~LED0;
			
			if(can_pid_cnt>=5)   //读出 5 种 ISO15765 值。
				can_pid_cnt=0;
